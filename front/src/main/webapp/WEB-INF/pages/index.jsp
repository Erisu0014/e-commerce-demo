...
<meta name="viewport"
	...
	<link href="${resourceServer }/css/seastyle.css" rel="stylesheet"
		type="text/css" />

	<script type="text/javascript"
		src="${resourceServer }/basic/js/jquery-1.7.min.js"></script>
	<script type="text/javascript" src="${resourceServer }/js/script.js"></script>
...
<body>
	<div id="app" v-cloak>
		...
		<div class="am-container header">
			...
			<ul class="message-r">
				...
				<div class="topMessage mini-cart">
					<div class="menu-hd">
						<a id="mc-menu-hd" href="${basepath }/shopcart.do" target="_top"><i
					...
					<div class="am-u-sm-12 am-u-md-12">
						<div class="theme-popover">
							...
							<ul class="select">
								...
								<div class="clear"></div>
								<template
									v-if="selectbrand.id!=0||selectcolor.id!=0||selectstorage.id!=0">
								<li class="select-result" style="display: list-item;">
									<dl>
										<dt>已选</dt>
										<dd class="select-no" style="display: none;"></dd>
										<p class="eliminateCriteria" @click="clear">清除</p>
										<dd v-if="selectbrand.id!=0" class="selected">
											<a href="javascript:void(0)" @click="selectbrand={id:0}">{{selectbrand.name}}</a>
										</dd>
										<dd v-if="selectcolor.id!=0" class="selected">
											<a href="javascript:void(0)" @click="selectcolor={id:0}">{{selectcolor.name}}</a>
										</dd>
										<dd v-if="selectstorage.id!=0" class="selected">
											<a href="javascript:void(0)" @click="selectstorage={id:0}">{{selectstorage.name}}</a>
										</dd>
									</dl>
								</li>
								</template>
								...
								<li class="select-list">
									<dl id="select1">
										...
										<div class="dd-conent">
											...
											<dd :class="{selected:selectbrand.id==brand.id}">
												<a href="javascript:void(0)" @click="selectbrand=brand">{{brand.name}}</a>
											</dd>
											</template>
										</div>
									</dl>
								...
								<li class="select-list">
									<dl id="select1">
										...
										<div class="dd-conent">
											...
											<dd :class="{selected:selectcolor.id==color.id}">
												<a href="javascript:void(0)" @click="selectcolor=color">{{color.name}}</a>
											</dd>
											</template>
										</div>
									</dl>
								...
								<li class="select-list">
									<dl id="select1">
										<dt class="am-badge am-round">内存</dt>
										<div class="dd-conent">
											<template v-for="storage in storages">
											<dd :class="{selected:selectstorage.id==storage.id}">
												<a href="javascript:void(0)" @click="selectstorage=storage">{{storage.name}}</a>
											</dd>
											</template>
										</div>
									</dl>
						...
						<div class="search-content">
							...
							<ul class="am-avg-sm-2 am-avg-md-3 am-avg-lg-4 boxes">
								<li v-for="product in products" @click="detail(product.id)">
									<div class="i-pic limit">
										<img :src="'http://172.18.0.27/'+image(product.images)"
											height="218" width="218"></img>
										<!-- <img :src="${resourceServer}/image(product.images)" /> -->
						...
						<%-- <div class="search-side">
							...
							<li>
								<div class="i-pic check">
									<img src="${resourceServer}/images/cp.jpg" />
							...
							<li>
								<div class="i-pic check">
									<img src="${resourceServer}/images/cp2.jpg" />
		...
		<script type="text/javascript">
			...
			$(function() {
				$.ajax({
					url : "${basepath}/product/getData.do",
					data : "data",
					dataType : "json",
					async : false,
				...
				var vue = new Vue(
						{
							...
							computed : {
								products : function() {
									var product;
									var colorId = this.selectcolor.id;
									var brandId = this.selectbrand.id;
									var storageId = this.selectstorage.id;
									$
											.ajax({
												url : "${basepath}/product/getProducts.do",
												async : false,
												dataType : "json",
												type : "get",
												data : {
							...
							methods : {
								...
								image : function(imagelist) {
									return imagelist.split(" ")[0];
								},
								detail : function(id) {
									window.location.href = "${basepath}/getProduct.do?id="
											+ id;
								}
							}
						});
